...
class BIMODAL():
    ...
    def train(self, data, extend_data, label, epochs=1, batch_size=1): #需要改，多了extend数据参数train(self, data, extend_data, label, epochs=1, batch_size=1)
        '''Train the model
        ...
        for i in range(epochs):
            ...
            for n in range(n_iter):
                ...
                # Current batch
                batch_data = torch.from_numpy(data[:, batch_start:batch_end, :].astype('float32')).to(self._device) # 需要改：下一行添加batch_extend_data = torch.from_numpy(extend_data[:, batch_start:batch_end, :].astype('float32')).to(self._device)
                batch_extend_data = torch.from_numpy(extend_data[batch_start:batch_end, :].astype('float32')).to(self._device)#####
                ...
                for j in range(self._molecule_size - 1):
                    ...
                    # Predict next token
                    #pred = self._lstm(batch_data[start:end], dir, self._device) # 需要改，self._lstm(batch_data[start:end], batch_extend_data[start:end], dir, self._device)
                    pred = self._lstm(batch_data[start:end], batch_extend_data, dir, self._device) # batch_extend_data去掉了[start:end]!!!!!!!!!!!!!!
    ...
    def validate(self, data, extend_data, label, batch_size=128):#需要改，多了extend数据参数train(self, data, extend_data, label, epochs=1, batch_size=1)
        ...
        with torch.no_grad():
            ...
            for n in range(n_iter):
                ...
                for j in range(self._molecule_size - 1):
                    ...
                    # Predict next token
                    pred = self._lstm(batch_data[start:end], batch_extend_data, dir, self._device)# [start:end]需要去掉!!!!!!!!!!!!!!!!!需要改，self._lstm(batch_data[start:end], batch_extend_data[start:end], dir, self._device)
    ...
    def sample(self, middle_token, T=1):
        ...
        with torch.no_grad():
            ...
            extend_feature = torch.zeros(1, 4).to(self._device) # 生成6个特征!!!!!!!!!!!!!!! （ramdom(1,5), ramdom（2，8））
            ...
            for j in range(self._molecule_size - 1):
                ...
                pred = self._lstm(seq[start:end], extend_feature, dir, self._device) # 加上extend_feature!!!!!!!!!!!!
