...
class SMILESEncoder():
    ...
    def encode_from_file(self, name='data'): # 需要改，改成输出两个参数，一个data，另一个6个指标
        '''One-hot-encoding from .csv file
        :param name:    name of data file
        :return:    encoded data (data size, molecule size, allowed token size)
        '''

        # Read data 要加上6个特征的输入
        if os.path.isfile(name + '.csv'):
            data = pd.read_csv(name + '.csv', header=None).values[:, 0]
            data = data.reshape(data.shape[0], -1)
            extend_six = pd.read_csv(name + '.csv', header=None).values[:,1:5]
        ...
        print(len(self._tokens), extend_six, data)
        return self.encode(data).reshape((shape[0], shape[1], -1, len(self._tokens))), extend_six   #加上另外六个指标
