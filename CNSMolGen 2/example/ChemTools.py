...
# 假设 numericalSort 和 smiles_stats 函数已经定义

def novelty_vs_epochs(folder, ft_path='../data/First_processed_SSRI.csv', pt_path='../data/First_processed.csv', export=False):
    ...
    # 打开一个文件用于保存统计信息
    log_file_path = os.path.join(folder, 'novelty_stats.log')
    with open(log_file_path, 'w') as log_file:
        ...
        for filename in sorted(os.listdir(folder_mols), key=numericalSort):  # imports in epoch order
            if filename.endswith(".csv"):
                ...
                # 提取当前文件的 valid、unique 和 novel 的值
                valid_val = stats['valid'].values[0]
                unique_val = stats['unique'].values[0]
                novel_val = stats['novel'].values[0]
                # 打印当前文件的 valid、unique 和 novel 的值
                print(f"File: {filename}, Valid: {valid_val}, Unique: {unique_val}, Novel: {novel_val}")
                # 将信息写入文件
                log_file.write(f"File: {filename}, Valid: {valid_val}, Unique: {unique_val}, Novel: {novel_val}\n")
                # updates the results
...
def smiles_stats(suppl, ft_path='../data/First_processed_SSRI.csv', pt_path='../data/First_processed.csv'):
    ...
    smis = []
    for mol in suppl:         #遍历 suppl（RDKit 的分子供应器），将每个分子转换为规范化的 SMILES 字符串，并存储在 smis 列表中。
        if mol is not None:
...
def morgan_vs_epochs(folder, path_ft='../data/First_processed_SSRI.csv'):
    # given a folder containing SMILES files, reads the files iteratively and computes the frequency of unique scaffolds
...
def morgan_similarity(path_denovo, path_ft='../data/First_processed_SSRI.csv'):
