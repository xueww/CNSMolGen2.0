...
class Trainer():
    ...
    def __init__(self, experiment_name='ForwardRNN'):
        ...
        self._data, self._data_extend = self._encoder.encode_from_file(self._file_name)     # 需要改：self._data，self._data_extend = self._encoder.encode_from_file(self._file_name)，extend就代表6个指标，encode_from_file需要改
    ...
    def complete_run(self, stor_dir='../evaluation/', restart=False):
        ...
        self._data, label, self._data_extend = shuffle(self._data.reshape(-1, self._molecular_size, self._encoding_size),
                                    label.reshape(-1, self._molecular_size), self._data_extend) # 需要改，需要保持smiles和6特征一一对应
        ...
        for i in range(self._epochs):
            ...
            # Train model
            statistic = self._model.train(self._data, self._data_extend, label, epochs=1, batch_size=self._batch_size) # 需要改：statistic = self._model.train(self._data, self._data_extend, label, epochs=1, batch_size=self._batch_size)，train函数需要改
    ...
    def single_run(self, stor_dir='../evaluation/', restart=False):
        ...
        train_data, test_data, train_data_extend, test_data_extend, train_label, test_label = train_test_split(self._data, self._data_extend, label, test_size=1. / 5,
                                                                          random_state=1, shuffle=True)  # 需要改
        # Build model
        self._model.build()
        ...
        for i in range(self._epochs):
            ...
            # to  (all_SMILES, molecular_size, encoding_size))
            statistic = self._model.train(train_data.reshape(-1, self._molecular_size, self._encoding_size), train_data_extend, 
                                          train_label.reshape(-1, self._molecular_size), epochs=1,
                                          batch_size=self._batch_size) # 需要改
            tot_stat.append(statistic.tolist())
            ...
            tot_loss.append(
                self._model.validate(test_data.reshape(-1, self._molecular_size, self._encoding_size), test_data_extend, 
                                     test_label.reshape(-1, self._molecular_size))) # 需要改，对应validate函数也需要改
    ...
    def cross_validation(self, stor_dir='../evaluation/', restart=False):
        ...
        # Split data into train and test data
        for train, test in self._kf.split(self._data): # 需要改

            # Shuffle index within test and train set
            np.random.shuffle(train) # 需要改
            np.random.shuffle(test) # 需要改
            ...
            for i in range(self._epochs):
                ...
                statistic = self._model.train(
                    self._data[train].reshape(-1, self._molecular_size, self._encoding_size),self._data_extend[train], 
                    label[train].reshape(-1, self._molecular_size), epochs=1, batch_size=self._batch_size) # 需要改
                ...
                # Test model on validation set
                tot_loss.append(
                    self._model.validate(self._data[test].reshape(-1, self._molecular_size, self._encoding_size), self._data_extend[test], # 这里有个.没写  !!!!!!!!!!!!!
                                         label[test].reshape(-1, self._molecular_size))) # 需要改
