...
class BiDirLSTM(nn.Module):
    ...
    def __init__(self, input_dim=110, hidden_dim=256, layers=2):
        ...
        self._wpred = nn.Linear(2 * self._hidden_dim+4, self._output_dim) # 需要+6!!!!!!!!!!!
    ...
    def forward(self, input, extend_input, next_prediction='right', device="cpu"): # 需要改：forward(self, input, extend_input, next_prediction='right', device="cpu")
        '''Forward computation
        ...
        norm_1 = torch.cat((norm_1, extend_input), -1)#####
